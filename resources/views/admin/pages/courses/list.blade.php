        ...
        <div class="container-fluid my-2">
            <div class="row mb-2">
                ...
                <div class="col-sm-6 text-right">
                    <a href="{{ route('course.create') }}" class="btn btn-primary">دورة جديدة</a>
                </div>
            </div>
        </div>
    </section>
                ...
                <div class="card-body table-responsive p-0">
                    <table class="table table-hover text-nowrap">
                        ...
                        <tbody>
                            ...
                            $interval = $firstDate->diff($lastDate)->format('%R%a يوم');
                            @endphp
                            <tr>
                                <td>{{ $item->id }}</td>
                                ...
                                <td>{{ $interval }}</td>
                                <td>{{ count($item->TargetGroups) > 2 ? "عدد الفئات المستهدفة ( " . count($item->TargetGroups) ." )" : $item->TargetGroups()->with('getGroup')->first()->getGroup->name }}</td>
                                <td>
                                    @if ($item->status == 1)
                    ...
                    @if($courses->isEmpty())
                        <div class="text-center my-4">لم يتم العثور على أي دورات</div>
                    @endif
                </div>
